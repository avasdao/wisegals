...
const init = () => {
    ...
    case 'ruth':
        ...
        title.value = `Ruth Laurent`
        nickname.value = `Ruth`
        specialty.value = `Restaurateur`
        summary.value = `Managing all of your Staking needs with ease.`
        profile.value = `Ruth's Chris Steak House was born in New Orleans when single mother Ruth Fertel went all-in on her vision of woning a restaurant she found for sale while browsing the classified ads.`
        break
    default:
        wisegalid.value = null
        title.value = `Bot Missing`
        nickname.value = `Bot`
        specialty.value = `A NEW Wisegal Bot is coming soon...`
        summary.value = `TBD...`
    }
}
...
<template>
    <main class="max-w-5xl mx-auto py-5 flex flex-col gap-4">
        <section class="flex flex-col gap-4">
            ...
            <h3 class="-mt-3 pl-2 text-fuchsia-600 text-xl font-medium tracking-widest">
                {{specialty}}
            </h3>

            <p class="text-gray-600 text-lg leading-8 tracking-wider">
                {{summary}}
            </p>
        </section>
        ...
        <section>
            <h3>
                Contracts Supported by {{nickname}}
            </h3>
        ...
        <WisegalMarie v-if="wisegalid === 'marie'" />
        <WisegalRuth v-if="wisegalid === 'ruth'" />
